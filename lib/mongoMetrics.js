...
function MongoMetrics (mongoUrl, collection) {
	...
	/*
		...
		@param {String} metricsAggregator - aggregation function to be used for aggregating metric of the each source
	...
	*/
	this.aggregate = function aggregate(name, resolution, metricsAggregator, sourceAggregator, query, callback) {
		...
		if(!MongoMetrics.AGGREGATORS[metricsAggregator]) {
			return callback(new Error("metricsAggregator is not supported"));
		}
		...
		var aggreation = [
			{$match: filter},
			{$group: this._generateMetricsGrouping(resolution, metricsAggregator)},
	...
	this._generateMetricsGrouping = function _generateMetricsGrouping(resolution, aggregator) {
