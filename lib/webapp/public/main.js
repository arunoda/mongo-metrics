$(function() {
	var gm = new GraphModel(['chunks', 'mp3'], { metric: 'avg', source: 'avg' }, 'five_secs', 1000 * 60 * 5);
...
function GraphModel(metrics, aggregators, resolution, timeLength) {
	...
	this.draw = function draw(cssSelector) {
		
		graph = new Graph(cssSelector, metrics, maxDataPoints, resolutionMillis);
...
function Graph(cssSelector, metrics, maxDataPoints, interval) {
...
MappedArray.prototype.update = function() {
	...
	fillGaps(this._array, this._interval);
	padAndTrim(this._array, this._size);
};
...
function padAndTrim(points, size) {
